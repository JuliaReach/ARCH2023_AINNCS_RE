...
print(io, "DoublePendulum,less robust,falsified,$(res.time)\n")

res = @timed run(false);
res_false = res.value

print(io, "DoublePendulum,more robust,falsified,$(res.time)\n")
...
savefig(fig, joinpath(TARGET_FOLDER, "Double-Pendulum-less-robust-x$(vars[1])-x$(vars[2]).png"))
...
plot_helper!(fig, vars, sol, sim, prob, spec)
savefig(fig, joinpath(TARGET_FOLDER, "Double-Pendulum-more-robust-x$(vars[1])-x$(vars[2]).png"))
